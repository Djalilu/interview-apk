...
const HomeScreen: React.FC = () => {
    ...
    const handleStartInterview = async (e: React.FormEvent) => {
        ...
        try {
            const { chat, firstQuestion } = await geminiService.startChat(company, jobRole, normalizedUrl, context!.language);
            setChatSession(chat);
                ...
                <form onSubmit={handleStartInterview} className="bg-light-card dark:bg-dark-card p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 space-y-6">
                    <div>
                     ...
                     <div>
                        ...
                        <input type="text" id="companyUrl" value={companyUrl} onChange={e => setCompanyUrl(e.target.value)} placeholder="e.g., www.google.com" required className="mt-1 block w-full px-3 py-2 bg-transparent border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary"/>
                    </div>
